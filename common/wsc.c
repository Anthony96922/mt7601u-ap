			...
			if (pCredential->EncrType &
				...
				if ((pCredential->KeyIndex >= 1)
					...
					if (WepKeyLen == 5 || WepKeyLen == 13
						...
						if (WepKeyLen == 5
						    || WepKeyLen == 13) {
							pAd->SharedKey[CurApIdx]
							    [WepKeyId].KeyLen =
							    (UCHAR) WepKeyLen;
							memcpy(pAd->SharedKey
							       [CurApIdx]
							       [WepKeyId].Key,
							       pCredential->Key,
							       WepKeyLen);
			...
			if (pCredential->EncrType &
				...
				if (pCredential->KeyLength >= 8
					...
					NdisMoveMemory(pWscControl->WpaPsk,
						       pCredential->Key,
						       pWscControl->WpaPskLen);
					RT_CfgSetWPAPSKKey(pAd, (PSTRING)
							   pCredential->Key,
							   pWscControl->
							   WpaPskLen, (PUCHAR)
							   pApCliTab->Ssid,
		...
		for (i = 0; i < MAX_PBC_STA_TABLE_SIZE; i++) {
			...
			if (pWscStaPbcProbeInfo->Valid[i] &&
			    RTMP_TIME_AFTER(now,
					    ...
					    120 * OS_HZ)) {
				NdisZeroMemory(&
					       (pWscStaPbcProbeInfo->StaMacAddr
						[i][0]), MAC_ADDR_LEN);
		...
		for (tab_idx = 0; tab_idx < MAX_PBC_STA_TABLE_SIZE; tab_idx++) {
			if (RTMP_TIME_AFTER
			    (now,
			     ...
			     120 * OS_HZ)
			    ||
			    NdisEqualMemory(pWscStaPbcProbeInfo->StaMacAddr
					    [tab_idx], &ZERO_MAC_ADDR[0],
					    MAC_ADDR_LEN)) {
