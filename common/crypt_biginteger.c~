...
/*    if ((*pBI = (PBIG_INTEGER) kmalloc(sizeof(BIG_INTEGER), GFP_ATOMIC)) == NULL) { */
	if (*pBI == NULL) {
		DEBUGPRINT
		    ("BigInteger_Init: allocate %d bytes memory failure.\n",
		     (sizeof(BIG_INTEGER)));
		return;
	}
	/* End of if */
...
/*        if (((*pBI)->pIntegerArray = (UINT32 *) kmalloc(sizeof(UINT32)*ArrayLength, GFP_ATOMIC)) == NULL) { */
		if ((*pBI)->pIntegerArray == NULL) {
			DEBUGPRINT
			    ("BigInteger_AllocSize: allocate %d bytes memory failure.\n",
			     (sizeof(UINT32) * ArrayLength));
			return;
		}		/* End of if */
		(*pBI)->AllocSize = sizeof(UINT32) * ArrayLength;
	...
	if ((pFirstOperand->Signed * pSecondOperand->Signed) > 0) {
		...
		for (BIArrayIndex = 0;
			...
			if (BIArrayIndex < pFirstOperand->ArrayLength)
				Sum += (UINT64)
				    pFirstOperand->pIntegerArray[BIArrayIndex];
			...
			if (BIArrayIndex < pTempBI2->ArrayLength) {
				if ((*pBI_Result)->pIntegerArray[BIArrayIndex]
				    ...
				    (pTempBI2->pIntegerArray[BIArrayIndex] -
				     Carry)) {
					(*pBI_Result)->pIntegerArray
					    [BIArrayIndex] =
					    (*pBI_Result)->pIntegerArray
					    [BIArrayIndex] -
					    pTempBI2->pIntegerArray
					    [BIArrayIndex] - Carry;
					Carry = 0;
				} else {
					(*pBI_Result)->pIntegerArray
	...
	for (BIFirstIndex = 0; BIFirstIndex < pFirstOperand->ArrayLength;
		...
		if (FirstValue == 0) {
			...
			for (BISecondIndex = 0;
			     ...
			     BISecondIndex++) {
				SecondValue = ((UINT64)
					       pSecondOperand->pIntegerArray
					       [BISecondIndex]) * FirstValue;
				Sum =
				    (UINT64) ((*pBI_Result)->pIntegerArray
					      [BIFirstIndex + BISecondIndex] +
					      SecondValue + Carry);
				Carry = Sum >> 32;
