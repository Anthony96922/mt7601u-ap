...
INT RTMPGetKeyParameter(IN PSTRING key,
			OUT PSTRING dest,
			IN int destsize, IN PSTRING buffer, IN bool bTrimSpace)
{
...
INT RTMPGetKeyParameterWithOffset(IN PSTRING key,
				  ...
				  IN int destsize,
				  IN PSTRING buffer, IN bool bTrimSpace)
{
	PSTRING temp_buf1 = NULL, temp_buf2 =
	    NULL, start_ptr, end_ptr, ptr, offset = 0;
	int len;
	...
	for (idx = 0; idx < MAX_MBSSID_NUM(pAd); idx++) {
		...
		if (RTMPGetKeyParameter
			...
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
				...
				NdisMoveMemory(pAd->ApCfg.MBSSID
					       [idx].AccessControlList.
			     ...
			     IN PSTRING pValueStr, IN PSTRING pInput)
{
	...
	if (RTMPGetKeyParameter("HT_OpMode", pValueStr, 25, pInput, TRUE)) {
		...
		pAd->CommonCfg.RegTransmitSetting.field.HTMODE =
		    ((Value == HTMODE_GF) ? HTMODE_GF : HTMODE_MM);
	...
	if (RTMPGetKeyParameter("HT_EXTCHA", pValueStr, 25, pInput, TRUE)) {
		...
		pAd->CommonCfg.RegTransmitSetting.field.EXTCHA =
		    ((Value == 0) ? EXTCHA_BELOW : EXTCHA_ABOVE);
	...
	if (RTMPGetKeyParameter("HT_STBC", pValueStr, 25, pInput, TRUE)) {
		...
		pAd->CommonCfg.RegTransmitSetting.field.STBC =
		    ((Value == STBC_USE) ? STBC_USE : STBC_NONE);
...
/*#endif*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if (RTMPGetKeyParameter
				...
				if (simple_strtol(tmpbuf, 0, 10) != 0) {	/* Enable */
					ChannelSel_Alg SelAlg = (ChannelSel_Alg)
					    simple_strtol(tmpbuf, 0, 10);
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
				...
				if (RTMPGetKeyParameter
					...
					NdisZeroMemory(&pAd->ApCfg.MBSSID
						       [i].WscControl.
						       WscDefaultSsid,
						       sizeof
						       (NDIS_802_11_SSID));
			...
			for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
				...
				if (RTMPGetKeyParameter
					...
					NdisMoveMemory(&pWpsCtrl->Wsc_Uuid_Str
						       [0], tmpbuf,
						       strlen(tmpbuf));
					DBGPRINT(RT_DEBUG_TRACE,
						 ("UUID_Str[%d] = %s\n", i + 1,
...
if (retval < 0) {
	...
	for (card_index = 0; card_index < MAX_NUM_OF_MULTIPLE_CARD;
	     card_index++) {
	...
	if ((card_same_mac_id >= 0) &&
		...
		if (card_select_method == MC_SELECT_CARDTYPE) {
			...
			snprintf(card_id_buf, sizeof(card_id_buf),
				 "%02dCARDTYPE%s", card_match_id, RFIC_word);
			...
			for (card_index = 0;
				...
				if ((start_ptr =
					...
					if (MC_CardUsed[card_index] == 0) {
						...
						if ((*(UINT32 *) &
						     MC_CardMac[card_index][0]
						     == 0)
						    && (*(UINT16 *) &
							MC_CardMac[card_index]
							[4] == 0)) {
		...
		for (readline = ptr = buffer, index = 0;
			...
			*ptr = '\0';
			...
			if (!strncmp(readline, "ch", 2)) {
				...
				if (StartCh == NULL) {
					StartCh = pwr;
					insertTailList(&pAd->SingleSkuPwrList,
						       (PLIST_ENTRY) pwr);
				} else {
					BOOLEAN isSame = TRUE;
