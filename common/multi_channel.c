...
//      RTMP_SEM_EVENT_UP(&pAd->reg_atomic2);   
	...
	if (edca_to_hcca)
		RTMPHwSendNullFrame(pAd,
				    ...
				    (pAd->CommonCfg.bWmmCapable & pAd->
				     CommonCfg.APEdcaParm.bValid), PWR_SAVE, 0);

	if (hcca_to_edca)
		RTMPHwSendNullFrame(pAd,
				    pAd->CommonCfg.TxRate,
				    (pAd->CommonCfg.bWmmCapable & pAd->
				     CommonCfg.APEdcaParm.bValid), PWR_SAVE, 1);
	...
	if (edca_to_hcca)
		RTMPHwSendNullFrame(pAd,
				    pAd->CommonCfg.TxRate,
				    (pAd->CommonCfg.bWmmCapable & pAd->
				     CommonCfg.APEdcaParm.bValid), PWR_ACTIVE,
				    1);
...
//      MACBuffer_Change(pAd,TRUE, FALSE);
	...
	RTMPHwSendNullFrame(pAd,
			    ...
			    (pAd->CommonCfg.bWmmCapable & pAd->CommonCfg.
			     APEdcaParm.bValid), PWR_ACTIVE, 0);
...
VOID MultiChannelSwitchToP2P(IN PRTMP_ADAPTER pAd)
{
	...
	RTMPHwSendNullFrame(pAd,
			    pAd->CommonCfg.TxRate,
			    (pAd->CommonCfg.bWmmCapable & pAd->CommonCfg.
			     APEdcaParm.bValid), PWR_ACTIVE, 1);
