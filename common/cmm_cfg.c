...
INT RT_CfgSetMbssWirelessMode(RTMP_ADAPTER * pAd, PSTRING arg)
{
	...
	if (WMODE_CAP_5G(wmode) && WMODE_CAP_2G(wmode)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("AP cannot support 2.4G/5G band mixed mode!\n"));
		return FALSE;
	}
	...
	case CMD_RTPRIV_IOCTL_FREQ_LIST_GET:
		{
			...
			for (i = 1; i <= pAd->ChannelListNum; i++) {
				m = 2412000;
				MAP_CHANNEL_ID_TO_KHZ(pAd->ChannelList[i -
								       1].
						      Channel, m);
				(*pFreq) = m;
				pFreq++;
	...
	case CMD_RTPRIV_IOCTL_VIRTUAL_INF_UP:
		/* interface up */
		{
			...
			if (VIRTUAL_IF_NUM(pAd) == 0) {
				...
				extern VOID APMakeAllBssBeacon(IN PRTMP_ADAPTER
							       pAd);
		...
		/* get wireless statistics */
		{
			...
			if (CurOpMode == OPMODE_AP) {
				...
				if ((pStats->priv_flags == INT_APCLI)) {
					INT ApCliIdx = ApCliIfLookUp(pAd,
								     (PUCHAR)
								     pStats->
								     dev_addr);
					if ((ApCliIdx >= 0)
					    && VALID_WCID(pAd->ApCfg.ApCliTab
							  [ApCliIdx].
							  MacTabWCID))
						pMacEntry =
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIWRATEQ:
		/* handle for SIOCGIWRATEQ */
		{
			...
			if (pRate->priv_flags == INT_WDS)
				HtPhyMode =
				    pAd->WdsTab.WdsEntry[pObj->ioctl_if].
				    HTPhyMode;
			else
#endif				/* WDS_SUPPORT */
			{
