			    ...
			    IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	sprintf(msg + strlen(msg), "%-4s%-33s%-20s%-23s%-9s%-7s%-7s%-3s\n",
		"Ch", "SSID", "BSSID", "Security", "Signal(%)", "W-Mode",
		" ExtCH", " NT");
...
#endif				/* DOT11_N_SUPPORT */
	default:
		snprintf(pBuf, BufLen, "\tUnknown Value(%d)",
			 pAd->CommonCfg.PhyMode);
		break;
	}
...
INT Show_HtGi_Proc(IN PRTMP_ADAPTER pAd, OUT PSTRING pBuf, IN ULONG BufLen)
{
	...
	case GI_800:
		...
		break;
	default:
		snprintf(pBuf, BufLen, "\tUnknown Value(%u)",
			 pAd->CommonCfg.RegTransmitSetting.field.ShortGI);
		break;
	}
	return 0;
...
INT Show_WpsUuid_Proc(IN PRTMP_ADAPTER pAd, OUT PSTRING pBuf, IN ULONG BufLen)
{
	...
	for (scIndex = 0; scIndex < maxCarriers; scIndex++) {
		for (i = 0; i < pProfData->rows; i++) {
			...
			if (pProfData->columns > 1) {
				ival = (CHAR) (pProfData->data[scIndex]
					       [6 * i + 2]);
				qval = (CHAR) (pProfData->data[scIndex]
					       [6 * i + 3]);
				col2Power[i] += ival * ival + qval * qval;
			}
