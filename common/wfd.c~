...
INT Set_WfdInsertIe_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (pAd->pCfg80211_CB != NULL) {
		...
		if (pCfg80211_CB->pCfg80211_Wdev != NULL) {
			if ((pCfg80211_CB->pCfg80211_Wdev->wiphy->
			     interface_modes & BIT(NL80211_IFTYPE_P2P_CLIENT))
			    || (pCfg80211_CB->pCfg80211_Wdev->wiphy->
				...
				case SUBID_WFD_ASSOCIATED_BSSID:
					{
						RTMPMoveMemory
						    (&pWfdEntryInfo->
						     wfd_serv_disc_query_info.
						     wfd_associate_bssid_ie,
						     pWfdEid->Octet,
						     SUBID_WFD_ASSOCIATED_BSSID_LEN);
						RTMPMoveMemory(&pWfdEntryInfo->
							       assoc_addr,
							       pWfdEid->Octet,
							       MAC_ADDR_LEN);
						DBGPRINT(RT_DEBUG_INFO,
