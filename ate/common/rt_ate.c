	...
	if (pAd->TxPowerCtrl.bInternalTxALC == TRUE) {
		{
			...
			if (pAd->TxPowerCtrl.bExtendedTssiMode == TRUE) {	/* Per-channel TSSI */
				if ((pATEInfo->Channel >= 1)
					...
					desiredTSSI +=
					    pAd->TxPowerCtrl.PerChTxPwrOffset
					    [pATEInfo->Channel];
				}
			}
...
INT Set_ATE_Cal_Free_Info_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (CurrentChannel = 3; CurrentChannel <= 14; CurrentChannel += 4) {
		...
		if (pAd->bUseEfuse) {
			if (pAd->bFroceEEPROMBuffer)
				NdisMoveMemory(&
					       (pAd->EEPROMImage
						[(EEPROM_TX_POWER_OFFSET_OVER_CH_3 + ((CurrentChannel - 3) / 2))]), (PUCHAR) (&EEPData), 2);
			else
				eFuseWrite(pAd,
...
INT Set_ATE_Read_TSSI_DC_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR BbpReg;
