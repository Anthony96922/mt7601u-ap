...
INT ATESetUpFrame(IN PRTMP_ADAPTER pAd, IN UINT32 TxIdx)
{
	...
	if (pNullContext->InUse == FALSE) {
		...
		RTMPZeroMemory(&pAd->NullContext[0].TransferBuffer->field.
			       WirelessPacket[0], TX_BUFFER_NORMSIZE);
		pTxInfo =
		    (TXINFO_STRUC *) & pAd->NullContext[0].TransferBuffer->
		    field.WirelessPacket[0];
		...
		pTxWI =
		    (TXWI_STRUC *) & pAd->NullContext[0].TransferBuffer->field.
		...
		if (pATEInfo->bQATxStart == TRUE) {
			...
			ATEWriteTxWI(pAd, pTxWI, pATEInfo->TxWI.TxWIFRAG, pATEInfo->TxWI.TxWITS, pATEInfo->TxWI.TxWIAMPDU, pATEInfo->TxWI.TxWIACK, pATEInfo->TxWI.TxWINSEQ, pATEInfo->TxWI.TxWIBAWinSize, BSSID_WCID, pATEInfo->TxWI.TxWIMPDUByteCnt	/* include 802.11 header */
				     ,
		...
		hex_dump("ATE",
			 pAd->NullContext[0].TransferBuffer->field.
			 WirelessPacket, 24);
		dump_txinfo(pAd, pTxInfo);
		dumpTxWI(pAd, pTxWI);

		RTMPMoveMemory(&pAd->NullContext[0].TransferBuffer->field.
	...
	hex_dump("ATE TX",
		 &pAd->NullContext[0].TransferBuffer->field.WirelessPacket[0],
