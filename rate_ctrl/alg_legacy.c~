...
VOID APMlmeDynamicTxRateSwitching(RTMP_ADAPTER * pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (IS_ENTRY_WDS(pEntry)
		    && !WDS_IF_UP_CHECK(pAd, pEntry->MatchWDSTabIdx))
			continue;
#endif				/* WDS_SUPPORT */
		...
		if ((pCurrTxRate->Mode <= MODE_CCK)
			...
			while (TmpIdx < TableSize) {
				...
				if (pEntry->SupportCCKMCS[pTmpTxRate->CurrMCS]
				    == TRUE) {
					UpRateIdx = TmpIdx;
					break;
				}
			...
			while (TmpIdx < TableSize) {
				pTmpTxRate =
				    PTX_RA_LEGACY_ENTRY(pTable, TmpIdx);
				if (pEntry->SupportOFDMMCS[pTmpTxRate->CurrMCS]
				    == TRUE) {
					UpRateIdx = TmpIdx;
					break;
				}
			...
			if ((CurrRateIdx > 0)
				...
				while (TmpIdx < TableSize) {
					pTmpTxRate =
					    PTX_RA_LEGACY_ENTRY(pTable, TmpIdx);
					if (pEntry->SupportHTMCS
					    [pTmpTxRate->CurrMCS] == TRUE) {
						UpRateIdx = TmpIdx;
						break;
					}
					TmpIdx++;
