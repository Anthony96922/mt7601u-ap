...
VOID ApMlmeDynamicTxRateSwitchingAGS(IN RTMP_ADAPTER * pAd, IN INT idx)
{
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: QuickAGS: AccuTxTotalCnt = %ld, TxSuccess = %ld, TxRetransmit = %ld, TxFailCount = %ld, TxErrorRatio = %ld\n",
		  __FUNCTION__, AGSStatisticsInfo.AccuTxTotalCnt,
		  AGSStatisticsInfo.TxSuccess, AGSStatisticsInfo.TxRetransmit,
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: AGS: Rssi = %d, TxSuccess = %lu, TxRetransmit = %lu, TxFailCount = %lu, TxErrorRatio = %lu\n",
		  __FUNCTION__, AGSStatisticsInfo.RSSI,
		  AGSStatisticsInfo.TxSuccess, AGSStatisticsInfo.TxRetransmit,
		  AGSStatisticsInfo.TxFailCount,
		  AGSStatisticsInfo.TxErrorRatio));
	...
	if (AGSStatisticsInfo.AccuTxTotalCnt <= 15) {
		...
		if (pTable == AGS3x3HTRateTable) {
			if (MCS[23]
			    && (AGSStatisticsInfo.RSSI > (-67 + RssiOffset)))
				TxRateIdx = MCS[23];
			else if (MCS[22]
			...
			if (MCS[15]
			    && (AGSStatisticsInfo.RSSI > (-69 + RssiOffset)))
				TxRateIdx = MCS[15];
			else if (MCS[14]
				 && (AGSStatisticsInfo.RSSI >
				     (-71 + RssiOffset)))
			...
			if (pTable == Ags1x1VhtRateTable)
				RssiOffset =
				    AgsRssiOffsetTable[0][pAd->CommonCfg.
							  BBPCurrentBW];
#endif				/* DOT11_VHT_AC */
			DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
			...
			/* 1x1 peer device (Adhoc, DLS or AP) */
			if (MCS[7]
			    && (AGSStatisticsInfo.RSSI > (-71 + RssiOffset)))
				TxRateIdx = MCS[7];
			else if (MCS[6]
				 && (AGSStatisticsInfo.RSSI >
				     (-73 + RssiOffset)))
		...
		pNextTxRate =
		    (RTMP_RA_LEGACY_TB
		     *) (&pTable[(pEntry->CurrTxRateIndex +
				  1) * SIZE_OF_AGS_RATE_TABLE_ENTRY]);
		...
		RTMPZeroMemory(pEntry->PER,
			       (sizeof(UCHAR) * (MAX_TX_RATE_INDEX + 1)));
	...
	do {
		...
		pEntry->PER[CurrRateIdx] =
		    (UCHAR) (AGSStatisticsInfo.TxErrorRatio);
		...
		if (bTrainUpDown) {
			...
			DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
				     ("%s: AGS: bTrainUpDown = %d, CurrRateIdx = %d, DownRateIdx = %d, UpRateIdx = %d, pEntry->TxQuality[CurrRateIdx] = %d, pEntry->TxQuality[UpRateIdx] = %d\n",
				      __FUNCTION__,
				      bTrainUpDown, CurrRateIdx, DownRateIdx,
				      UpRateIdx, pEntry->TxQuality[CurrRateIdx],
				      pEntry->TxQuality[UpRateIdx]));
	...
	DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
		     ("%s: AGS: pEntry->CurrTxRateIndex = %d, CurrRateIdx = %d, pEntry->LastSecTxRateChangeAction = %d\n",
		      __FUNCTION__, pEntry->CurrTxRateIndex, CurrRateIdx,
...
VOID ApQuickResponeForRateUpExecAGS(IN RTMP_ADAPTER * pAd, IN INT idx)
{
	...
	do {
		...
		OneSecTxNoRetryOKRationCount =
		    (AGSStatisticsInfo.TxSuccess * ratio);
