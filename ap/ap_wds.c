...
BOOLEAN ApWdsAllowToSendPacket(IN RTMP_ADAPTER * pAd,
...
LONG WdsEntryAlloc(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr)
{
	INT i;
	LONG WdsTabIdx = -1;
	...
	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		...
		if (pAd->WdsTab.WdsEntry[i].Valid == FALSE) {
			...
			pAd->WdsTab.Size++;
			COPY_MAC_ADDR(pAd->WdsTab.WdsEntry[i].PeerWdsAddr,
				      pAddr);
			WdsTabIdx = i;
			break;
		} else
		    if (MAC_ADDR_EQUAL
			(pAd->WdsTab.WdsEntry[i].PeerWdsAddr, pAddr)) {
			WdsTabIdx = i;
			break;
		}
	}
...
VOID WdsEntryDel(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr)
{
	...
	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		if (MAC_ADDR_EQUAL(pAddr, pAd->WdsTab.WdsEntry[i].PeerWdsAddr)
			...
			NdisZeroMemory(pAd->WdsTab.WdsEntry[i].PeerWdsAddr,
				       MAC_ADDR_LEN);
			pAd->WdsTab.Size--;
			break;
		}
...
/*
	...
	==========================================================================
 */
BOOLEAN MacTableDeleteWDSEntry(IN PRTMP_ADAPTER pAd,
...
================================================================
*/
MAC_TABLE_ENTRY *MacTableInsertWDSEntry(IN PRTMP_ADAPTER pAd,
	...
	do {
		...
		pEntry =
		    MacTableInsertEntry(pAd, pAddr,
					(WdsTabIdx + MIN_NET_DEVICE_FOR_WDS),
					OPMODE_AP, TRUE);
		if (pEntry) {
			...
			case MODE_HTMIX:
				...
				HTPhyMode.field.ShortGI =
				    pAd->WdsTab.WdsEntry[WdsTabIdx].HTPhyMode.
				    field.ShortGI;
				HTPhyMode.field.BW =
				    pAd->WdsTab.WdsEntry[WdsTabIdx].HTPhyMode.
				    field.BW;
			...
			pEntry->MinHTPhyMode.word =
			    pAd->WdsTab.WdsEntry[WdsTabIdx].MinHTPhyMode.word;
			pEntry->HTPhyMode.word = pEntry->MaxHTPhyMode.word;

#ifdef DOT11_N_SUPPORT
			if (pAd->WdsTab.WdsEntry[WdsTabIdx].PhyMode >=
			    MODE_HTMIX) {
				if (pAd->WdsTab.WdsEntry[WdsTabIdx].
				...
				if (pAd->CommonCfg.bRdg
				    && pEntry->HTCapability.ExtHtCapInfo.
				    RDGSupport)
					CLIENT_STATUS_SET_FLAG(pEntry,
							       fCLIENT_STATUS_RDG_CAPABLE);
				if (pEntry->HTCapability.ExtHtCapInfo.
				    MCSFeedback == 0x03)
					CLIENT_STATUS_SET_FLAG(pEntry,
			...
			if (pAd->WdsTab.WdsEntry[WdsTabIdx].bAutoTxRateSwitch ==
			    FALSE) {
				pEntry->HTPhyMode.field.MCS =
				    pAd->WdsTab.WdsEntry[WdsTabIdx].
				    DesiredTransmitSetting.field.MCS;
				pEntry->bAutoTxRateSwitch = FALSE;
				/* If the legacy mode is set, overwrite the transmit setting of this entry. */
				RTMPUpdateLegacyTxSetting((UCHAR) pAd->WdsTab.
							  WdsEntry[WdsTabIdx].
							  DesiredTransmitSetting.
							  field.FixedTxMode,
							  pEntry);
			...
			pAd->WdsTab.WdsEntry[WdsTabIdx].MacTabMatchWCID =
			    (UCHAR) pEntry->Aid;
...
MAC_TABLE_ENTRY *WdsTableLookupByWcid(IN PRTMP_ADAPTER pAd,
				      IN UCHAR wcid,
				      IN PUCHAR pAddr,
...
MAC_TABLE_ENTRY *WdsTableLookup(IN PRTMP_ADAPTER pAd,
...
MAC_TABLE_ENTRY *FindWdsEntry(IN PRTMP_ADAPTER pAd,
			      IN UCHAR Wcid, IN PUCHAR pAddr, IN UINT32 PhyMode)
{
	...
	if ((pEntry == NULL) && (pAd->WdsTab.Mode >= WDS_LAZY_MODE)) {
		...
		if (WdsIdx >= 0) {
			...
			pEntry =
			    MacTableInsertWDSEntry(pAd, pAddr, (UCHAR) WdsIdx);
		...
		if (pEntry->NoDataIdleCount >= MAC_TABLE_AGEOUT_TIME) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("ageout %02x:%02x:%02x:%02x:%02x:%02x from WDS #%d after %d-sec silence\n",
				  ...
				  pEntry->Addr[4], pEntry->Addr[5], idx,
...
VOID RT28xx_WDS_Close(IN PRTMP_ADAPTER pAd)
{
	UINT index;
...
VOID WdsDown(IN PRTMP_ADAPTER pAd)
{
	int i;

	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		if (WdsTableLookup
		    (pAd, pAd->WdsTab.WdsEntry[i].PeerWdsAddr, TRUE))
			MacTableDeleteWDSEntry(pAd,
					       pAd->WdsTab.WdsEntry[i].
					       MacTabMatchWCID,
					       pAd->WdsTab.WdsEntry[i].
					       PeerWdsAddr);
	}
...
VOID AsicUpdateWdsRxWCIDTable(IN PRTMP_ADAPTER pAd)
{
	UINT index;
	...
	for (index = 0; index < MAX_WDS_ENTRY; index++) {
		...
		pEntry =
		    MacTableInsertWDSEntry(pAd,
					   pAd->WdsTab.WdsEntry[index].
					   PeerWdsAddr, index);
		...
		case MODE_HTGREENFIELD:
			pEntry->SupportRateMode =
			    (SUPPORT_HT_MODE | SUPPORT_OFDM_MODE |
			     SUPPORT_CCK_MODE);
			break;
#endif				/* DOT11_N_SUPPORT */
...
VOID AsicUpdateWdsEncryption(IN PRTMP_ADAPTER pAd, IN UCHAR wcid)
{
	...
	do {
		...
		if (((pAd->WdsTab.WdsEntry[WdsIdex].WepStatus ==
		      Ndis802_11Encryption1Enabled)
		     || (pAd->WdsTab.WdsEntry[WdsIdex].WepStatus ==
			...
			if (pAd->WdsTab.WdsEntry[WdsIdex].WepStatus ==
			    Ndis802_11Encryption1Enabled)
				DefaultKeyId =
				    pAd->WdsTab.WdsEntry[pEntry->
							 MatchWDSTabIdx].KeyIdx;
...
VOID WdsPeerBeaconProc(IN PRTMP_ADAPTER pAd,
...
VOID APWdsInitialize(IN PRTMP_ADAPTER pAd)
{
	INT i;
	...
	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		...
		pAd->WdsTab.WdsEntry[i].WepStatus =
		    Ndis802_11EncryptionDisabled;
		pAd->WdsTab.WdsEntry[i].KeyIdx = 0;
		NdisZeroMemory(&pAd->WdsTab.WdsEntry[i].WdsKey,
			       sizeof(CIPHER_KEY));

		pAd->WdsTab.WdsEntry[i].bAutoTxRateSwitch = TRUE;
		pAd->WdsTab.WdsEntry[i].DesiredTransmitSetting.field.MCS =
		    MCS_AUTO;
	}
	return;
}

INT Show_WdsTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT i;

	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		DBGPRINT(RT_DEBUG_OFF,
			 ("IF/WDS%d-%02x:%02x:%02x:%02x:%02x:%02x(%s) ,%s, KeyId=%d\n",
			  i, PRINT_MAC(pAd->WdsTab.WdsEntry[i].PeerWdsAddr),
			  pAd->WdsTab.WdsEntry[i].Valid ==
			  1 ? "Valid" : "Invalid",
...
VOID rtmp_read_wds_from_file(IN PRTMP_ADAPTER pAd,
			     PSTRING tmpbuf, PSTRING buffer)
{
	...
	if (RTMPGetKeyParameter("WdsEncrypType", tmpbuf, 128, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("WdsEncrypType[%d]=%d(%s)\n", i,
				  pAd->WdsTab.WdsEntry[i].WepStatus,
				  GetEncryptType(pAd->WdsTab.WdsEntry[i].
						 WepStatus)));
		}
		...
		if (i == 1) {
			for (j = 1; j < MAX_WDS_ENTRY; j++) {
				pAd->WdsTab.WdsEntry[j].WepStatus =
				    pAd->WdsTab.WdsEntry[0].WepStatus;
				DBGPRINT(RT_DEBUG_TRACE,
					 ("@WdsEncrypType[%d]=%d(%s)\n", j,
					  pAd->WdsTab.WdsEntry[i].WepStatus,
					  GetEncryptType(pAd->WdsTab.WdsEntry
							 [i].WepStatus)));
			}
		}
	...
	if (RTMPGetKeyParameter("WdsKey", tmpbuf, 255, buffer, FALSE)) {
		...
		if (((pAd->WdsTab.WdsEntry[0].WepStatus ==
		      Ndis802_11Encryption2Enabled)
		     || (pAd->WdsTab.WdsEntry[0].WepStatus ==
			 Ndis802_11Encryption3Enabled))
			...
			if (pAd->WdsTab.WdsEntry[0].WepStatus ==
			    Ndis802_11Encryption3Enabled)
				pAd->WdsTab.WdsEntry[0].WdsKey.CipherAlg =
				    CIPHER_AES;
			else
				pAd->WdsTab.WdsEntry[0].WdsKey.CipherAlg =
				    CIPHER_TKIP;

			NdisMoveMemory(&pAd->WdsTab.WdsEntry[0].WdsKey.Key,
				       keyMaterial, 16);
			pAd->WdsTab.WdsEntry[0].WdsKey.KeyLen = 16;
			NdisMoveMemory(&pAd->WdsTab.WdsEntry[0].WdsKey.RxMic,
				       keyMaterial + 16, 8);
			NdisMoveMemory(&pAd->WdsTab.WdsEntry[0].WdsKey.TxMic,
				       keyMaterial + 16, 8);
		}
		...
		if (pAd->WdsTab.WdsEntry[0].WdsKey.KeyLen == 16) {
			for (j = 1; j < MAX_WDS_ENTRY; j++) {
				NdisMoveMemory(&pAd->WdsTab.WdsEntry[j].WdsKey,
					       &pAd->WdsTab.WdsEntry[0].WdsKey,
					       sizeof(CIPHER_KEY));
			}
		}
	...
	if (!bUsePrevFormat) {
		for (i = 0; i < MAX_WDS_ENTRY; i++) {
			...
			if (RTMPGetKeyParameter
				...
				if (pAd->WdsTab.WdsEntry[i].WepStatus ==
				    Ndis802_11Encryption1Enabled) {
					/* Ascii type */
					if (strlen(tmpbuf) == 5
					    || strlen(tmpbuf) == 13) {
						KeyLen = strlen(tmpbuf);
						pAd->WdsTab.WdsEntry[i].WdsKey.
						    KeyLen = KeyLen;
						NdisMoveMemory(pAd->WdsTab.
							       WdsEntry[i].
							       WdsKey.Key,
							       tmpbuf, KeyLen);
						if (KeyLen == 5)
					...
					pAd->WdsTab.WdsEntry[i].WepStatus =
					    Ndis802_11EncryptionDisabled;
					NdisZeroMemory(&pAd->WdsTab.WdsEntry[i].
						       WdsKey,
						       sizeof(CIPHER_KEY));
				}
			}
	...
	if (RTMPGetKeyParameter("WdsTxMode", tmpbuf, 25, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("I/F(wds%d) Tx Mode = %d\n", i,
				  pWdsEntry->DesiredTransmitSetting.field.
				  FixedTxMode));
		}
	}
		...
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			pWdsEntry->DesiredTransmitSetting.field.MCS =
			    RT_CfgSetTxMCSProc(macptr,
					       &pWdsEntry->bAutoTxRateSwitch);

			if (pWdsEntry->DesiredTransmitSetting.field.MCS ==
			    MCS_AUTO) {
				DBGPRINT(RT_DEBUG_TRACE,
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("I/F(wds%d) Tx MCS = %d\n", i,
					  pWdsEntry->DesiredTransmitSetting.
					  field.MCS));
			}
		}
	}
			...
			for (i = 0; i < MAX_WDS_ENTRY; i++) {
				...
				if (i > 0) {
					pAd->WdsTab.WdsEntry[i].WepStatus =
					    pAd->WdsTab.WdsEntry[0].WepStatus;
					pAd->WdsTab.WdsEntry[i].KeyIdx =
					    pAd->WdsTab.WdsEntry[0].KeyIdx;
					NdisMoveMemory(&pAd->WdsTab.WdsEntry[i].
						       WdsKey,
	...
	/* WdsVlan */
	if (RTMPGetKeyParameter
	    ("WDS_VLANID", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_WDS_ENTRY);
		     macptr = rstrtok(NULL, ";"), i++) {
			pAd->WdsTab.WdsEntry[i].VLAN_VID =
...
VOID WdsPrepareWepKeyFromMainBss(IN PRTMP_ADAPTER pAd)
{
	INT i;
	...
	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		...
		if (pAd->WdsTab.WdsEntry[i].WepStatus ==
			...
			UCHAR main_bss_keyid =
			    pAd->ApCfg.MBSSID[MAIN_MBSSID].DefaultKeyId;

			if (pAd->ApCfg.MBSSID[MAIN_MBSSID].WepStatus ==
			    Ndis802_11Encryption1Enabled
			    && (pAd->SharedKey[MAIN_MBSSID][main_bss_keyid].
				KeyLen == 5
				|| pAd->SharedKey[MAIN_MBSSID][main_bss_keyid].
				KeyLen == 13)) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Duplicate IF/WDS%d wep key from main_bssid \n",
					  (UCHAR) i));
				pAd->WdsTab.WdsEntry[i].KeyIdx = main_bss_keyid;
				NdisMoveMemory(&pAd->WdsTab.WdsEntry[i].WdsKey,
					       &pAd->SharedKey[MAIN_MBSSID]
					       [main_bss_keyid],
					       sizeof(CIPHER_KEY));
			} else {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("No available wep key for IF/WDS%d, reset its encryption as OPEN \n",
					  (UCHAR) i));
				pAd->WdsTab.WdsEntry[i].WepStatus =
				    Ndis802_11EncryptionDisabled;
				NdisZeroMemory(&pAd->WdsTab.WdsEntry[i].WdsKey,
...
VOID WDS_Init(IN PRTMP_ADAPTER pAd, IN RTMP_OS_NETDEV_OP_HOOK * pNetDevOps)
{
	INT index;
	PNET_DEV pWdsNetDev;
	...
	for (index = 0; index < MAX_WDS_ENTRY; index++) {
		...
		pWdsNetDev =
		    RtmpOSNetDevCreate(MC_RowID, &IoctlIF, INT_WDS, index,
				       sizeof(PRTMP_ADAPTER), INF_WDS_DEV_NAME);
#ifdef HOSTAPD_SUPPORT
		pAd->IoctlIF = IoctlIF;
#endif				/* HOSTAPD_SUPPORT */
		if (pWdsNetDev == NULL) {
			...
			pWdsEntry =
			    &pAd->MacTab.Content[pAd->WdsTab.WdsEntry
						 [index].MacTabMatchWCID];
			DBGPRINT(RT_DEBUG_TRACE,
				 ("The new WDS interface MAC = %02X:%02X:%02X:%02X:%02X:%02X\n",
				  PRINT_MAC(pWdsEntry->Addr)));
		}

		NdisZeroMemory(&pAd->WdsTab.WdsEntry[index].WdsCounter,
			       sizeof(WDS_COUNTER));
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Total Allocate %d WDS interfaces!\n", index));

}

int WDS_PacketSend(IN PNDIS_PACKET pSkb,
		   IN PNET_DEV dev, IN RTMP_NET_PACKET_TRANSMIT Func)
{
	UCHAR i;
	RTMP_ADAPTER *pAd;
...
VOID WDS_Remove(IN PRTMP_ADAPTER pAd)
{
	UINT index;
...
BOOLEAN WDS_StatsGet(IN PRTMP_ADAPTER pAd, IN RT_CMD_STATS * pStats)
{
	...
	if (index >= MAX_WDS_ENTRY) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("rt28xx_ioctl can not find wds I/F\n"));
		return FALSE;
	}
	...
	pStats->rx_packets =
	    pAd->WdsTab.WdsEntry[WDS_apidx].WdsCounter.ReceivedFragmentCount.
	    QuadPart;
	pStats->tx_packets =
	    pAd->WdsTab.WdsEntry[WDS_apidx].WdsCounter.TransmittedFragmentCount.
	    QuadPart;
