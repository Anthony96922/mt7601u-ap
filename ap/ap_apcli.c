...
========================================================================
*/
VOID RT28xx_ApCli_Close(IN PRTMP_ADAPTER ad_p)
{
	UINT index;
		     ...
		     INOUT ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	aux_ht_cap->HtCapInfo.TxSTBC =
	    (rt_ht_cap->TxSTBC) & (pHtCapability->HtCapInfo.RxSTBC);
	aux_ht_cap->HtCapInfo.RxSTBC =
	    (rt_ht_cap->RxSTBC) & (pHtCapability->HtCapInfo.TxSTBC);
...
BOOLEAN ApCliLinkUp(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex)
{
	...
	do {
		if (ifIndex < MAX_APCLI_NUM) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("!!! APCLI LINK UP - IF(apcli%d) AuthMode(%d)=%s, WepStatus(%d)=%s !!!\n",
				  ...
				  GetAuthMode(wdev->AuthMode), wdev->WepStatus,
		...
		if (pMacEntry) {
			...
			if ((wdev->AuthMode == Ndis802_11AuthModeWPA2) &&
			    (NdisEqualMemory
			     (pAd->MlmeAux.Bssid, pApCliEntry->LastBssid,
			      MAC_ADDR_LEN) == FALSE)
			    && (pApCliEntry->bLostAp == TRUE)) {
				pApCliEntry->bLostAp = FALSE;
			}

			COPY_MAC_ADDR(pApCliEntry->LastBssid,
				      pAd->MlmeAux.Bssid);
	...
	if ((pMacEntry->AuthMode == Ndis802_11AuthModeWPA2) &&
	    (NdisEqualMemory
	     (pAd->MlmeAux.Bssid, pAd->ApCfg.ApCliTab[ifIndex].LastBssid,
	      MAC_ADDR_LEN))
	...
	for (index = 0; index < MAX_APCLI_NUM; index++) {
		...
		if (pApCliEntry->Valid == TRUE) {
			...
			if ((pMacEntry->AuthMode >= Ndis802_11AuthModeWPA)
			    ...
			    (RTMP_TIME_AFTER
			     (pAd->Mlme.Now32,
			      (pApCliEntry->ApCliLinkUpTime + (30 * OS_HZ)))))
				bForceBrocken = TRUE;
		...
		case IE_RSN:
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("ApCliValidateRSNIE - RSN-WPA2 PairWiseCipher(%s), GroupCipher(%s), AuthMode(%s)\n",
				  (WPA2.bMixMode ? "Mix" :
				   GetEncryptType(WPA2.PairCipher)),
		...
		if (pAd->chipCap.MBSSIDMode == MBSSID_MODE1) {
			...
			pApCliEntry->CurrentAddress[ETH_LENGTH_OF_ADDRESS - 1] =
			    (pApCliEntry->CurrentAddress
			     [ETH_LENGTH_OF_ADDRESS - 1] + pAd->ApCfg.BssidNum +
			     MAX_MESH_NUM) & 0xFF;
		}
