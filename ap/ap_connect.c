...
VOID APMakeBssBeacon(RTMP_ADAPTER * pAd, INT apidx)
{
	...
	MakeOutgoingFrame(pBeaconFrame, &FrameLen,
			  ...
			  1, &DsIe,
			  1, &DsLen, 1, &pAd->CommonCfg.Channel, END_OF_ARGS);
...
#else
			{
				UCHAR MaxTxPower = GetCountryMaxTxPwr(pAd,
								      pAd->
								      CommonCfg.
								      Channel);
...
/*		USHORT	b2lTmp, b2lTmp2; // no use */
		...
		*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
		...
		if ((pComCfg->Channel <= 14) &&
			...
			OverlapScanParam.PassiveTalPerChannel =
			    cpu2le16
			    (pComCfg->Dot11OBssScanPassiveTotalPerChannel);
			OverlapScanParam.ActiveTalPerChannel =
			    cpu2le16
			    (pComCfg->Dot11OBssScanActiveTotalPerChannel);
	...
	/* add Ralink-specific IE here - Byte0.b0=1 for aggregation, Byte0.b1=1 for piggy-back */
	{
		...
		IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x00, 0x00,
			    0x00, 0x00};
	...
	if ((pAd->ApCfg.DtimCount == 0) &&
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("DTIM Period reached, BSS20WidthReq=%d, Intolerant40=%d!\n",
			  ...
			  pAd->CommonCfg.LastBSSCoexist2040.
			  field.Intolerant40));
