				 ...
				 IN CHAR Rssi1, IN CHAR Rssi2, IN UCHAR AntSel)
{
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		if (NdisEqualMemory
		    (pHeader->Addr2, pAd->ApCfg.MBSSID[i].Bssid,
		     MAC_ADDR_LEN)) {
			CHAR RcvdRssi;

			RcvdRssi =
			    RTMPMaxRssi(pAd,
					ConvertToRssi(pAd, Rssi0, RSSI_0,
						      AntSel, BW_20),
			...
			case SUBTYPE_ASSOC_RSP:
				pAd->ApCfg.MBSSID[i].
				    RcvdSpoofedAssocRespCount++;
				pAd->ApCfg.MBSSID[i].
				    RssiOfRcvdSpoofedAssocResp = RcvdRssi;
				break;
			...
			case SUBTYPE_REASSOC_RSP:
				...
				pAd->ApCfg.MBSSID[i].
				    RssiOfRcvdSpoofedReassocResp = RcvdRssi;
				break;
			...
			case SUBTYPE_PROBE_RSP:
				...
				pAd->ApCfg.MBSSID[i].
				    RssiOfRcvdSpoofedProbeResp = RcvdRssi;
				break;
			...
			default:
				...
				pAd->ApCfg.MBSSID[i].
				    RssiOfRcvdSpoofedUnknownMgmt = RcvdRssi;
				break;
			       ...
			       IN CHAR Rssi1, IN CHAR Rssi2, IN UCHAR AntSel)
{
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		if (SSID_EQUAL
		    (pSsid, SsidLen, pAd->ApCfg.MBSSID[i].Ssid,
...
VOID rtmp_read_ids_from_file(IN PRTMP_ADAPTER pAd,
			     PSTRING tmpbuf, PSTRING buffer)
{
	...
	if (RTMPGetKeyParameter
	    ("AssocReqFloodThreshold", tmpbuf, 10, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("ReassocReqFloodThreshold", tmpbuf, 10, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("ProbeReqFloodThreshold", tmpbuf, 10, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("DisassocFloodThreshold", tmpbuf, 10, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("DeauthFloodThreshold", tmpbuf, 10, buffer, TRUE)) {
	...
	if (RTMPGetKeyParameter
	    ("EapReqFloodThreshold", tmpbuf, 10, buffer, TRUE)) {
