...
/*		if ((PID & 0x3f) < WDS_PAIRWISE_KEY_OFFSET) // send to client */
		{
			MgtMacHeaderInit(pAd, pNullFr, SUBTYPE_NULL_FUNC, 0,
					 pAddr, pAd->ApCfg.MBSSID[apidx].Bssid);
...
VOID APOverlappingBSSScan(IN RTMP_ADAPTER * pAd)
{
	...
	{			/* For 2.4GHz band */
		...
		if (index < pAd->ChannelListNum) {
			...
			if (pAd->CommonCfg.RegTransmitSetting.field.EXTCHA ==
				...
				chEndIdx =
				    ((curSecChIdx + 3) <
				     pAd->ChannelListNum) ? (curSecChIdx + 3)
				    : (pAd->ChannelListNum - 1);
			} else {
				curPriChIdx = index;
				curSecChIdx =
				...
				chEndIdx =
				    ((curPriChIdx + 3) <
				     pAd->ChannelListNum) ? (curPriChIdx + 3)
				    : (pAd->ChannelListNum - 1);;
			}
		} else {
			/* It should not happened! */
	...
	for (index = chStartIdx; index <= chEndIdx; index++) {
		...
		if ((pAd->ChannelList[index].bEffectedChannel &
