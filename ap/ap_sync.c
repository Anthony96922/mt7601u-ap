	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		if ((pAd->CommonCfg.ExtRateLen) && (PhyMode != WMODE_B)) {
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  ...
					  pAd->CommonCfg.ExtRate, 1, &ErpIe, 1,
					  &ErpIeLen, 1,
		...
		if (WMODE_CAP_N(PhyMode) &&
			...
			if (pAd->ApCfg.MBSSID[apidx].bWmmCapable) {
				UCHAR i;
				UCHAR WmeParmIe[26] =
				    { IE_VENDOR_SPECIFIC, 24, 0x00, 0x50, 0xf2,
					0x02, 0x01, 0x01, 0, 0
				};
				WmeParmIe[8] =
				    pAd->ApCfg.BssEdcaParm.
				    EdcaUpdateCount & 0x0f;
#ifdef UAPSD_SUPPORT
...
*/
			...
			} else if (ie_list->Channel != pAd->CommonCfg.Channel)
				goto __End_Of_APPeerBeaconAction;
		}
#endif				/* DOT11_N_SUPPORT */
...
VOID APScanTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pAd->MlmeAux.ScanType == SCAN_WSC_ACTIVE
