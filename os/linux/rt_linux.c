...
VOID RtmpOsSetNetDevTypeMonitor(VOID * pDev) {
	RTMP_OS_NETDEV_SET_TYPE((PNET_DEV) pDev,
...
========================================================================
*/
		BOOLEAN RtmpOsSemaInitLocked(RTMP_OS_SEM * pSem,
					     LIST_HEADER * pSemList) {
			if (RTMP_OS_Alloc_RscOnly(pSem, sizeof(OS_SEM))
			    == FALSE) {
				DBGPRINT(RT_DEBUG_ERROR,
...
========================================================================
*/
		BOOLEAN RtmpOsSemaInit(RTMP_OS_SEM * pSem,
				       LIST_HEADER * pSemList) {
			if (RTMP_OS_Alloc_RscOnly(pSem, sizeof(OS_SEM))
			    == FALSE) {
				DBGPRINT(RT_DEBUG_ERROR,
...
*/
		VOID RtmpOsPktBodyCopy(PNET_DEV pNetDev,
			...
			memcpy(skb_put(pNetPkt, ThisFrameLen), pData,
			       ThisFrameLen);
			SET_OS_PKT_NETDEV(pNetPkt, pNetDev);
			RTMP_SET_PACKET_SOURCE(OSPKT_TO_RTPKT(pNetPkt),
					       PKTSRC_NDIS);
		}
...
*/
		PNDIS_PACKET RtmpOsPktClone(PNDIS_PACKET pNetPkt) {
			return skb_clone(RTPKT_TO_OSPKT(pNetPkt),
					 MEM_ALLOC_FLAG);
		}
...
========================================================================
*/
		VOID RtmpOsPktTailAdjust(PNDIS_PACKET pNetPkt,
					 UINT removedTagLen) {
...
*/
		VOID RtmpOsPktInfPpaSend(PNDIS_PACKET pNetPkt) {
			...
			memset(pRxPkt->head, 0,
			       pRxPkt->data - pRxPkt->head - 14);
			DBGPRINT(RT_DEBUG_TRACE,
			...
			ret =
			    ppa_hook_directpath_send_fn(pAd->g_if_id,
							pRxPkt,
							pRxPkt->len, ppa_flags);
		...
		long RtmpOsSimpleStrtol(const char *cp, char **endp,
					unsigned int base) {
		...
		BOOLEAN RtmpOsPktOffsetInit(VOID) {
			...
			if ((RTPktOffsetData == 0)
			    && (RTPktOffsetLen == 0)
			    && (RTPktOffsetCB == 0)) {
				pPkt =
				    kmalloc(sizeof(struct sk_buff), GFP_ATOMIC);
				if (pPkt == NULL)
					return FALSE;

				RTPktOffsetData =
				    (ULONG) (&(pPkt->data)) - (ULONG) pPkt;
				RTPktOffsetLen =
				    (ULONG) (&(pPkt->len)) - (ULONG) pPkt;
				RTPktOffsetCB =
				    (ULONG) (pPkt->cb) - (ULONG) pPkt;
				kfree(pPkt);
...
========================================================================
*/
		BOOLEAN RtmpOsAtomicInit(RTMP_OS_ATOMIC * pAtomic,
					 LIST_HEADER * pAtomicList) {
			if (RTMP_OS_Alloc_RscOnly
			    (pAtomic, sizeof(atomic_t)) == FALSE) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: alloc atomic fail!\n",
					  __FUNCTION__));
				return FALSE;	/* allocate fail */
...
========================================================================
*/
		VOID RtmpOsAtomicInterlockedExchange(RTMP_OS_ATOMIC *
						     pAtomicSrc, LONG Value) {
			if (pAtomicSrc->pContent)
				InterlockedExchange((atomic_t *)
						    (pAtomicSrc->pContent),
						    Value);
		}
