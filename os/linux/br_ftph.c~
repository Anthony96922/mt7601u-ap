...
UINT32 BG_FTPH_PacketFromApHandle(IN PNDIS_PACKET pPacket)
{
	...
	if (pNetDev != NULL) {
		if (pNetDev->br_port != NULL) {
			...
			pDstFdbEntry =
			    br_fdb_get_hook(pNetDev->br_port->br, pRxPkt->data);
			pSrcFdbEntry =
			    br_fdb_get_hook(pNetDev->br_port->br,
					    pRxPkt->data + 6);
#else
			...
			pSrcFdbEntry =
			    br_fdb_get(pNetDev->br_port->br, pRxPkt->data + 6);
#endif
			...
			if ((!pDstFdbEntry->is_local) &&
				...
				pDstFdbEntry->dst->dev->hard_start_xmit(pRxPkt,
									pDstFdbEntry->
									dst->
									dev);
				return 0;
