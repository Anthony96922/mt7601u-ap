			    ...
			    IN USHORT subcmd, IN VOID * pData, IN ULONG Data)
{
	...
	case CMD_RTPRIV_IOCTL_80211_ACTION_FRAME_REG:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("pAd->Cfg80211RegisterActionFrame=%d [%d]\n",
	...
	case CMD_RTPRIV_IOCTL_80211_CHANNEL_LIST_SET:
		DBGPRINT(RT_DEBUG_TRACE,
			 ("CMD_RTPRIV_IOCTL_80211_CHANNEL_LIST_SET: %d\n",
			  Data));
		...
		if (pChanList != NULL) {
			...
			os_alloc_mem(NULL, (UINT32 **) & pAd->pCfg80211ChanList,
				     sizeof(UINT32 *) * Data);
			if (pAd->pCfg80211ChanList != NULL) {
...
static VOID CFG80211DRV_DisableApInterface(VOID * pAdOrg)
{
	UINT32 Value;
...
BOOLEAN CFG80211DRV_OpsBeaconSet(VOID * pAdOrg, VOID * pData, BOOLEAN isAdd)
{
	...
	ssid_ie =
	    cfg80211_find_ie(WLAN_EID_SSID, pBeacon->beacon + 36,
			     pBeacon->beacon_len - 36);
#endif
	...
	if (isAdd) {
		...
		RTMPMakeRSNIE(pAd, Ndis802_11AuthModeWPA2PSK,
			      Ndis802_11Encryption3Enabled, MAIN_MBSSID);
...

		{
			...
			Addr4 =
			    (ULONG) (pP2PBssid[4]) | (ULONG) (pP2PBssid[5] <<
							      8);
	...
	RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, TRUE, FALSE, FALSE, TRUE, 0,
		      BSS0Mcast_WCID, pBeacon->beacon_len, PID_MGMT, 0, 0,
		...
		for (IdChan = 0; IdChan < ChanNum; IdChan++) {
			...
			for (IdPwr = 0; IdPwr < MAX_NUM_OF_CHANNELS; IdPwr++) {
				if (ChanId == pAd->TxPower[IdPwr].Channel) {
					...
					/* keep DFS type */
					pAd->ChannelList[RecId].RegulatoryDomain
					    = DfsType;
...
INT CFG80211_ApStaDel(IN VOID * pAdCB, IN UCHAR * pMac)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdCB;
	MAC_TABLE_ENTRY *pEntry;
...
INT CFG80211_setDefaultKey(IN VOID * pAdCB, IN UINT Data)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdCB;
