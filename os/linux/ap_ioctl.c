...
INT rt28xx_ap_ioctl(IN struct net_device *net_dev,
...
/*    				strcpy((PSTRING) wrq->u.name, (PSTRING) pAd->ApCfg.MBSSID[pObj->ioctl_if].Bssid); */
	...
	case SIOCGIWESSID:	/*Get ESSID */
		{
			RT_CMD_AP_IOCTL_SSID IoctlSSID, *pIoctlSSID =
			    &IoctlSSID;
			...
			RTMP_AP_IoctlHandle(pAd, wrq,
					    CMD_RTPRIV_IOCTL_AP_SIOCGIWESSID, 0,
					    pIoctlSSID, 0);
			...
			if ((erq->pointer) && (pSsidStr != NULL)) {
				...
				if (copy_to_user
				    (erq->pointer, pSsidStr, erq->length)) {
...
/*				PCHAR pBssidStr; */
			...
			RTMP_AP_IoctlHandle(pAd, wrq,
					    CMD_RTPRIV_IOCTL_AP_SIOCGIWAP, 0,
					    wrqin->u.ap_addr.sa_data,
					    RT_DEV_PRIV_FLAGS_GET(net_dev));
		}
		break;
	case SIOCGIWMODE:	/*get operation mode */
		wrqin->u.mode = IW_MODE_MASTER;	/*SoftAP always on INFRA mode. */
		break;
...
/*				struct iw_range range; */
			...
			/* allocate memory */
			os_alloc_mem(NULL, (UCHAR **) & prange,
			...
			len =
			    copy_to_user(wrq->u.data.pointer, prange,
					 sizeof(struct iw_range));
	...
	case RT_PRIV_IOCTL_EXT:
		{
			...
			Status =
			    RTMP_AP_IoctlHandle(pAd, wrq, CMD_RT_PRIV_IOCTL,
						subcmd, wrqin->u.data.pointer,
						0);
		}
		break;
	...
	case SIOCGIWPRIV:
		if (wrqin->u.data.pointer) {
			...
			if ((sizeof(ap_privtab) / sizeof(ap_privtab[0])) <=
			    wrq->u.data.length) {
				wrqin->u.data.length =
				    sizeof(ap_privtab) / sizeof(ap_privtab[0]);
				if (copy_to_user
				    (wrqin->u.data.pointer, ap_privtab,
				     sizeof(ap_privtab)))
					Status = RTMP_IO_EFAULT;
			} else
